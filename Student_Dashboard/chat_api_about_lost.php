...
$stmt = $con->prepare("
    SELECT lf.id,lf.name, lf.student_id AS owner_id
    FROM lost_founds lf
    JOIN students s ON lf.student_id = s.id
    WHERE lf.id = ? AND lf.status = 1 AND s.active = 1
    LIMIT 1
");
...
$stmt = $con->prepare("
    ...
    WHERE r.item_id = ? AND rm.student_id = ?
    LIMIT 1
");
...
// Insert new room with item_id
$stmt = $con->prepare("INSERT INTO rooms (title, description, created_by, item_id) VALUES (?, ?, ?, ?)");
$stmt->bind_param('ssii', $title, $description, $user_id, $item_id);
$stmt->execute();
